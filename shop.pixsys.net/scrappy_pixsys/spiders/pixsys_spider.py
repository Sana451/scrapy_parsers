...
class PixsysSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        error_product_urls = []
        with open(
                "/home/sana451/PycharmProjects/scrapy_parsers/shop.pixsys.net/scrappy_pixsys/results/pixsys_links_179.csv") as csvfile:
    ...
    def parse_product(self, response, error_product_urls):
        ...
        data["Заголовок"] = response.css("h1::text").get()
        data["Модель/Тип"] = response.css("span.navigation_page::text").get()
        data["Condition"] = response.css("p#product_condition span::text").get()
        data["Reference"] = response.css("span.editable:nth-child(2)::text").get()
        data["Цена"] = response.css("div.box-info-product #our_price_display").attrib["content"]
        data["Наличие"] = response.css("#availability_value::text").get()
        data["Картинки"] = response.css("img#bigpic").attrib["src"]
        data["Краткое описание"] = get_short_description(response)
        data["Характеристики"] = get_specifications(response)
        data["Категории"] = " > ".join(response.css("span.navigation_page a span::text").getall())

        try:
            description_link = response.css("div#short_description_content a").attrib["href"]
            if description_link:
